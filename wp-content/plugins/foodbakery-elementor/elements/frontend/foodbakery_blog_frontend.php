...
class Foodbakery_Blog_Frontend {
    ...
    public function render($settings) {
        ...
        if (isset($foodbakery_blog_cat) && $foodbakery_blog_cat <> '' && $foodbakery_blog_cat <> '0') {
            $blog_category_array = array('category_name' => implode(',',$foodbakery_blog_cat));
        ...
        if ($foodbakery_blog_cat != '' && $foodbakery_blog_cat != '0') {
            ...
            $row_cat = $wpdb->get_row($wpdb->prepare("SELECT * from $wpdb->terms WHERE slug = %s", implode(',',$foodbakery_blog_cat)));
        }
