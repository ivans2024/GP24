...
if (!function_exists('foodbakery_payment_summary_fields')) {
     ...
     * @return html
     */
    function foodbakery_payment_summary_fields($get_trans_id) {

        global $current_user;

        $user_info = get_user_info_array();
        
        $user_info = apply_filters('foodbakery_payment_summary_user_data', $user_info, $get_trans_id);
        $delivery_address_extras = apply_filters('foodbakery_payment_summary_adress_field_extras', '');
				...
				<div class="row">
					...
					<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
						<div class="field-holder">
							...
							<textarea '. $delivery_address_extras .' class="foodbakery-dev-req-field" name="trans_address">' . $user_info['address'] . '</textarea>
						</div>
					</div>
				</div>
			</div>
        ...
        return apply_filters('foodbakery_payment_summary_fields', $html, $get_trans_id);
        ...
        if (is_array($gateways) && sizeof($gateways) > 0) {
            ...
            if (is_user_logged_in() && $publisher_type != 'buyer' && $get_action == 'reservation-order') {
                ...
                if ($transaction_detail_bank != 'true') {
					...
					<div class="row">' . foodbakery_payment_summary_fields($get_trans_id);
            ...
            if ($order_type == 'order' && is_array($order_menu_list)) {
                ...
                foreach ($order_menu_list as $_menu_list) {
                    ...
                    $price_item = (isset($_menu_list['price']) && $_menu_list['price'] > 0) ? $_menu_list['price'] : 0;
                    ...
                    if (is_array($extras_item) && sizeof($extras_item) > 0) {
                        ...
                        foreach ($extras_item as $extra_item) {
                            ...
                            $menu_item_title = isset($extra_item['menu_item_title']) ? $extra_item['menu_item_title'] : '';
                    ...
                    $payment_gw_list .= apply_filters('order_item_detail_after_title', '', $_menu_list);
                ...
                if ($order_m_total > 0) {
                    ...
                    $payment_gw_list = apply_filters('foodbakery_calculations_area_right_frontend', $payment_gw_list, $order_m_total, $transaction_id);
                    $payment_gw_list .= '
							</ul>';
                    $payment_gw_list = apply_filters('foodbakery_order_checkout_field_before_total', $payment_gw_list, $transaction_id, restaurant_menu_price_calc('defined', $order_m_total, $menu_order_fee, true, false, false, $transaction_id, false, false));
			$payment_gw_list .= '<p class="total-price">' . esc_html__('Total', 'foodbakery') . ' <span class="price" data-order_total_price="'.restaurant_menu_price_calc('defined', $order_m_total, $menu_order_fee, true, false, false, $transaction_id, false).'">' . restaurant_menu_price_calc('defined', $order_m_total, $menu_order_fee, true, false, false, $transaction_id, true) . '</span></p>
						</div>';
                }
                $payment_gw_list .= '
					</div> 
...
if (!function_exists('foodbakery_payment_process')) {
    ...
    function foodbakery_payment_process($foodbakery_transaction_fields = array()) {
        ...
        if (isset($transaction_amount) && isset($transaction_pay_method) && $transaction_amount > 0 && $transaction_pay_method != '') {
            ...
            if( isset( $trans_order_id )){
                $menu_order_fee = get_post_meta($trans_order_id, 'menu_order_fee', true);
                $order_subtotal_price = get_post_meta($trans_order_id, 'order_subtotal_price', true);
            }
            ...
            if ($trans_id) {
                ...
                do_action('foodbakery_order_process_submit', $trans_id, $_POST);
                do_action('foodbakery_order_service_total_fee', $trans_id);
                ...
                $trans_meta_arr = apply_filters('foodbakery_trans_meta_array', $trans_meta_arr, $_POST);                
                ...
                $trans_order_id = isset( $trans_order_id )? $trans_order_id : 0;
                $order_subtotal_price = isset( $order_subtotal_price)? $order_subtotal_price : 0;
                $menu_order_fee = isset( $menu_order_fee)? $menu_order_fee : 0;
                
                update_post_meta($transaction_id, 'transaction_amount', restaurant_menu_price_calc('defined', $order_subtotal_price, $menu_order_fee, true, false, false, $trans_order_id, false));
                ...
                if ($transaction_order_type == 'reservation-order') {
                    ...
                    update_post_meta($trans_order_id, 'foodbakery_order_show', 1);
                    update_post_meta($trans_order_id, 'foodbakery_new_order_notice', 1);
                    do_action('foodbakery_order_confirmation', $trans_order_id, $trans_order_id, array(), array());
                    ...
                    if ($trans_charges_amount != '') {
                        ...
                        $trans_fee_meta_arr = apply_filters('foodbakery_trans_meta_array', $trans_fee_meta_arr, $_POST);
                ...
                update_option('trans_id', $trans_id);
                update_option('trans_item_id', $trans_item_id);
                update_option('transaction_id', $transaction_id);
                
                $foodbakery_order_id    = isset( $foodbakery_transaction_fields['foodbakery_order_id'] )? $foodbakery_transaction_fields['foodbakery_order_id'] : 0;
                $foodbakery_transaction_ptype   = get_post_meta( $foodbakery_order_id, 'foodbakery_transaction_ptype', true);
                if( $foodbakery_transaction_ptype != 'add-restaurant'){
                    $foodbakery_transaction_fields = apply_filters('foodbakery_transaction_fields', $foodbakery_transaction_fields, $_POST, $transaction_id);
                }
                ...
                if ($transaction_pay_method == 'FOODBAKERY_PAYPAL_GATEWAY' && !empty($foodbakery_transaction_fields)) {
                    ...
                    update_post_meta($trans_order_id, 'foodbakery_order_show', 1);
                    update_post_meta($trans_order_id, 'foodbakery_new_order_notice', 1);
                    do_action('foodbakery_order_confirmation_woocommerce', $trans_order_id);
                    do_action('foodbakery_order_confirmation', $trans_order_id, $trans_order_id, array(), array());
