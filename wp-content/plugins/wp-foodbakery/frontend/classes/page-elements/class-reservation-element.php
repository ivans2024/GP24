    ...
    class foodbakery_reservation_element {
        ...
        public function foodbakery_restaurant_add_menu_item() {
            ...
            if (!is_user_logged_in()) {
                ...
                if ($publisher_id != '') {
                    ...
                    if ($publisher_type != 'restaurant' && $publisher_type != '') {
                        ...
                        do_action('foodbakery_menu_item_cart_before_adding', $restaurant_id, $get_added_menus, $menu_item_id);
                        ...
                        if ($menu_updating == 'true') {
                            ...
                            if (isset($get_added_menus[$restaurant_id][$menu_index])) {
                                ...
                                $updated_menu = apply_filters('foodbakery_add_menu_item_cart', $updated_menu, $restaurant_menu_list, $menu_item_id);
                                $get_added_menus[$restaurant_id][$menu_index] = $updated_menu;
                            }
                        } else {
                            if (is_array($get_added_menus) && sizeof($get_added_menus) > 0) {

                                $updated_menu_array = array(
                                    'menu_cat_id' => $menu_cat_id,
                                    'menu_id' => $menu_item_id,
                                    'price' => $this_item_price,
                                    'unique_id' => $rand_numb,
                                ...
                                $updated_menu_array = apply_filters('foodbakery_add_menu_item_cart', $updated_menu_array, $restaurant_menu_list, $menu_item_id);
                                
                                $get_added_menus[$restaurant_id][] = $updated_menu_array;
                            } else {
                                $get_added_menus = array();
                                $updated_menu_array = array(
                                    'menu_cat_id' => $menu_cat_id,
				...
				<li class="menu-added-' . $rand_numb_class . '" id="menu-added-' . $rand_numb . '" data-pr="' . foodbakery_get_currency($menu_t_price, false, '', '', false) . '" data-conpr="' . foodbakery_get_currency($menu_t_price, false, '', '', true) . '">
					...
					<span class="category-price">' . foodbakery_get_currency($this_item_price, true) . '</span>';
                        ...
                        $menu_id_key = key(array_slice($get_added_menus[$restaurant_id], -1, 1, true));
                        $li_html .= apply_filters('foodbakery_cart_after_menu_item', '', $menu_id_key, $get_added_menus, $restaurant_id);
                        $li_html .= '
						</li>';
                        set_transient('add_menu_items_' . $publisher_id, $get_added_menus, 60 * 60 * 24 * 30);
            ...
            if ($jus_pl) {
                ...
                // menu title
                $this_item_title = isset($restaurant_menu_list[$menu_item_id]['menu_item_title']) ? $restaurant_menu_list[$menu_item_id]['menu_item_title'] : '';
                //pre($restaurant_menu_list, false);
                //pre($menu_item_id);
                ...
                if ($menu_updating == 'true') {
                    ...
                    if (is_array($get_added_menus) && sizeof($get_added_menus) > 0) {
                        ...
                        $get_added_menus[$restaurant_id][] = apply_filters('foodbakery_add_menu_item_cart', $updated_menu_array, $restaurant_menu_list, $menu_item_id);
                    }
                }
        ...
        public function foodbakery_restaurant_order_confirm() {
            ...
            $order_delivery_fee = foodbakery_get_input('order_delivery_fee', '');
            ...
            do_action('foodbaekery_confirm_order_process_before', $restaurant_id);
            ...
            if ($publisher_id != '' && $publisher_type != '' && $publisher_type != 'restaurant' || $user_id == '') {
                ...
                update_post_meta($order_id, 'foodbakery_order_show', 1);
                update_post_meta($order_id, 'foodbakery_new_order_notice', 1);
                
                if ($woocommerce_enabled == 'on') {
                    update_post_meta($order_id, 'foodbakery_order_show', 0);
                    update_post_meta($order_id, 'foodbakery_new_order_notice', 0);
                }else{
                    do_action('foodbakery_order_confirmation', $order_id, $restaurant_id, $menu_item_identity, $this_item_quantity);
                }
                ...
                if ($order_id) {
                    ...
                    if (isset($get_added_menus[$restaurant_id]) && is_array($get_added_menus[$restaurant_id])) {
                        ...
                        foreach ($get_added_menus[$restaurant_id] as $menu_item_do) {
                            //pre($menu_item_do);
                            $is_deal_item = isset($menu_item_do['deal_item']) ? $menu_item_do['deal_item'] : false;
                            $deal_id = isset($menu_item_do['deal_id']) ? $menu_item_do['deal_id'] : false;
                            
                            $menu_single_ar = array();
                            $menu_item_id = isset($menu_item_do['menu_id']) ? $menu_item_do['menu_id'] : '';
                            $menu_item_identity = isset($menu_item_do['menu_item_identity']) ? $menu_item_do['menu_item_identity'] : '';
                            $menu_item_extr = isset($menu_item_do['extras']) ? $menu_item_do['extras'] : '';
                            $this_item_quantity = isset($menu_item_do['quantity']) ? $menu_item_do['quantity'] : 1;
                            $menu_item_identity = isset($menu_item_do['menu_item_identity']) ? $menu_item_do['menu_item_identity'] : 1;
                            $unique_menu_id = isset($menu_item_do['unique_menu_id']) ? $menu_item_do['unique_menu_id'] : 1;
                            
                            $this_item_extras = isset($restaurant_menu_list[$menu_item_id]['menu_item_extra']) ? $restaurant_menu_list[$menu_item_id]['menu_item_extra'] : '';

                            if( $is_deal_item == false){
                            // menu category
                            ...
                            $this_item_price = isset($restaurant_menu_list[$menu_item_id]['menu_item_price']) ? $restaurant_menu_list[$menu_item_id]['menu_item_price'] : '';
                                ...
                                // menu category
                            $this_item_category = isset($menu_deals[$unique_menu_id]['restaurant_menu']) ? $menu_deals[$unique_menu_id]['restaurant_menu'] : '';

                            // menu title
                            $this_item_title = isset($menu_deals[$unique_menu_id]['deal_name']) ? $menu_deals[$unique_menu_id]['deal_name'] : '';

                            // menu price
                            $this_item_price = isset($menu_deals[$unique_menu_id]['deal_price']) ? $menu_deals[$unique_menu_id]['deal_price'] : '';
                            
                            $this_item_extras = isset($menu_item_do['extras']) ? $menu_item_do['extras'] : array();
                            
                                
                            }
                          
                            //pre($this_item_extras);
                            
                            $extras_arra = array();

                            // menu extras
                            $Foodbakery_deal_backend = new Foodbakery_deal_backend();
                           if (isset($menu_item_extr) && is_array($menu_item_extr) && sizeof($menu_item_extr) > 0) {
                                $menu_ext_counter = 0;
                                foreach ($menu_item_extr as $this_item_extra_at) {
                                    $menu_item_id  = isset($this_item_extra_at['menu_id']) ? $this_item_extra_at['menu_id'] : 0;
                                    
                                    $menu_item_data = $Foodbakery_deal_backend->foodbakery_get_menu_item($restaurant_id, $menu_item_id);
                                    
                                    
                                    $this_item_heading = isset($menu_item_data['menu_item_extra']['heading'][$this_item_extra_at['title_id']]) ? $menu_item_data['menu_item_extra']['heading'][$this_item_extra_at['title_id']] : '';
                                    ...
                                    $menu_extra_menu_id = isset($this_item_extra_at['menu_id']) ? $this_item_extra_at['menu_id'] : '';
                                    $menu_extra_menu_title = isset($this_item_extra_at['menu_item_title']) ? $this_item_extra_at['menu_item_title'] : '';
                                    ...
                                    $extras_arra[] = array(
                                        ...
                                        'title' => $menu_extra_at_label,
                                        'price' => $menu_extra_at_price,
                                        'menu_id' => $menu_extra_menu_id,
                                        'is_deal_item' => $is_deal_item,
                                        'restaurant_id' => $restaurant_id,
                                        'menu_item_title' => $menu_extra_menu_title,
                                        'deal_id'   => $deal_id,
                                    );
                                    //pre($extras_arra);
                                    $menu_ext_counter ++;
...
//                            }
                            ...
                            $menu_single_ar['menu_item_identity']    = $menu_item_identity;
                            $menu_single_ar['category'] = $this_item_category;
                            $menu_single_ar['title'] = $this_item_title;
                            $menu_single_ar['price'] = $this_item_price;
                            $menu_single_ar['quantity'] = $this_item_quantity;
                            $menu_single_ar['extras'] = $extras_arra;
                            $menu_single_ar['is_deal_item'] = $is_deal_item;
                            $menu_single_ar['restaurant_id'] = $restaurant_id;
                            $menu_single_ar['deal_id'] = $deal_id;
                            
                            $menu_single_ar = apply_filters('foodbakery_menu_order_array', $menu_single_ar, $menu_item_do);
                    ...
                    if(!$check_delivery_tax) {
                        ...
                        $foodbakery_delivery_fee    = apply_filters('foodbakery_resturant_delivery_fee', $foodbakery_delivery_fee, $restaurant_id);
                        ...
                        if ($selected_fee_type == 'delivery' && $foodbakery_delivery_fee > 0 && $foodbakery_pickup_fee > 0) {
                            ...
                            $menu_save_fee = isset( $order_delivery_fee )? $order_delivery_fee : $menu_save_fee;
                            $foodbakery_delivery_fee = apply_filters('foodbakery_retaurant_delivery_fee', $menu_save_fee, $restaurant_id);
                            $menu_save_fee = foodbakery_get_currency($foodbakery_delivery_fee, false, '', '', false);
                            ...
                            if ($foodbakery_delivery_fee >= 0 && $restaurant_pickup_delivery != 'pickup') {
                                $foodbakery_delivery_fee = apply_filters('foodbakery_retaurant_delivery_fee', $foodbakery_delivery_fee, $restaurant_id);
                                $menu_save_fee = foodbakery_get_currency($foodbakery_delivery_fee, false, '', '', false);
                                $menu_save_fee = isset( $order_delivery_fee )? $order_delivery_fee : $menu_save_fee;
                                $menu_save_fee_type = 'delivery';
