...
if (!class_exists('foodbakery_restaurant_meta')) {
	...
	public function foodbakery_restaurant_save_menu_category($restaurant_id = '') {
	    ...
	    $restaurant_menu_cat_icons = foodbakery_get_input('menu_cat_icon', '', 'ARRAY');
		...
		update_post_meta($restaurant_id, 'menu_cat_titles', $restaurant_menu_cat_titles);
            ...
            if (!empty($restaurant_menu_cat_icons)) {
		update_post_meta($restaurant_id, 'menu_cat_icons', $restaurant_menu_cat_icons);
	    }
	}
	    	...
	    	<div class="opt-conts">
	    		...
	    		<nav class="admin-navigtion">
	    			...
	    			<li><a href="javascript:void(0);" name="#tab-menu-categories" href="javascript:;"><i class="icon-list"></i> <?php esc_html_e('Menu Categories', 'foodbakery') ?></a></li>
                                ...
                                <?php echo  do_action('foodbakery_resturance_tab_item_backend'); ?>
	    		    </ul>
	    		</nav>
	    		<div id="tabbed-content" data-ajax-url="<?php echo esc_url(admin_url('admin-ajax.php')); ?>">
                    ...
                    <?php do_action('foodbakery_add_menu_coupon_content_inrestaurant_backend'); ?>
                            <?php echo  do_action('foodbakery_resturance_tab_item_content_backend', $post); ?>
	    		</div>
	...
	function foodbakery_restaurant_options() {
		...
		foreach ($cust_query as $foodbakery_restaurant_type) {
            ...
            echo '<div class="icon-image-browse-field">';
	    ...
	    do_action('foodbakery_indeed_restaurant_admin_fields');
            do_action('foodbakery_restaurant_admin_fields_after_menu', $post_id);
	...
	public function foodbakery_restaurant_cat_form($get_menu_cat_vals, $menu_item_counter, $doin_action = 'add') {
		...
		$desc_val = isset($get_menu_cat_vals['menu_cat_desc']) ? $get_menu_cat_vals['menu_cat_desc'] : '';
                $icon_val = isset($get_menu_cat_vals['menu_cat_icon']) ? $get_menu_cat_vals['menu_cat_icon'] : '';
		$add_btn_func = ' onClick="foodbakery_close_menu_cat(\'' . $menu_item_counter . '\');"';
                $icon_field_name = 'menu_cat_icon';
	    } else {
		...
		$add_btn_func = ' onClick="foodbakery_admin_add_menu_cat_to_list(\'' . $menu_item_counter . '\');"';
                $icon_field_name = 'menu_cat_icon_add';
	    }
            $icon_html = foodbakery_iconlist_plugin_options($icon_val, 'menu_cat_icon_'.$menu_item_counter, $icon_field_name); 
				...
				<div class="field-holder">
					...
					<input class="menu-item-title" data-js_id="_menu_cat_title" id="menu_item_title_' . $menu_item_counter . '"' . $title_name_value . ' type="text" placeholder="' . esc_html__('Menu Category Title', 'foodbakery') . '">	
                        ...
                        <div class="col-lg-2 col-md-2 col-sm-12 col-xs-12">
                                <div class="field-holder">
					<label>' . esc_html__('Menu Category Icon *', 'foodbakery') . '</label>
                                        '. $icon_html .'
                                </div>
			</div>';
            
            $form_html  .= apply_filters('foodbakery_menu_category_fields','', $menu_item_counter, $get_menu_cat_vals, $doin_action);
	...
	public function foodbakery_restaurant_menu_cat_item($menu_item_counter = '', $menu_cat_vals = array()) {
		...
		$menu_cat_desc = isset($_POST['_menu_cat_desc']) ? $_POST['_menu_cat_desc'] : '';
                $menu_cat_icon = isset($_POST['_menu_cat_icon']) ? $_POST['_menu_cat_icon'] : '';
	...
	public function group_restaurant_menu_cats($restaurant_id) {
	    ...
	    $restaurant_menu_cat_icons = get_post_meta($restaurant_id, 'menu_cat_icons', true);
		...
		foreach ($restaurant_menu_cat_titles as $cat_title) {
		    ...
		    $cat_desc = isset($restaurant_menu_cat_descs[$cat_counter]) ? $restaurant_menu_cat_descs[$cat_counter] : '';
                    $cat_icon = isset($restaurant_menu_cat_icons[$cat_counter]) ? $restaurant_menu_cat_icons[$cat_counter] : '';
		    $get_menu_cat_vals = array(
	...
	function restaurant_fields($restaurant_type_slug = 0, $post_id = 0) {
	    ...
	    $html .= $foodbakery_html_fields->foodbakery_select_field($foodbakery_opt_array);
            $html = apply_filters('foodbakery_restaurant_field_backend',$html);
	...
	function restaurant_opening_hours($restaurant_type_slug = 0, $post_id = 0) {
		...
		if (is_array($week_days) && sizeof($week_days) > 0) {
			...
			if (isset($get_opening_hours) && is_array($get_opening_hours) && sizeof($get_opening_hours) > 0) {
			    $_opening_time = get_post_meta($post_id, 'foodbakery_opening_hours_' . $day_key . '_opening_time', true);
			    $opening_time = ( $_opening_time != '' ? date('h:i a', $_opening_time) : '' );
                            $opening_time = apply_filters('opening_hours_open_time_format', $opening_time, $_opening_time);
                            
                            
			    $_closing_time = get_post_meta($post_id, 'foodbakery_opening_hours_' . $day_key . '_closing_time', true);
			    $closing_time = ( $_closing_time != '' ? date('h:i a', $_closing_time) : '' );
                            $closing_time = apply_filters('opening_hours_close_time_format', $closing_time, $_closing_time);
							...
							<div id="open-close-con-' . $day_key . '-' . $restaurant_add_counter . '" class="open-close-time' . (isset($day_status) && $day_status == 'on' ? ' opening-time' : '') . '">
								...
								<div class="time-sec">
									...
									<select class="chosen-select " name="foodbakery_opening_hour[' . $day_key . '][closing_time]">
										' . $time_to_html . '
									</select>'.apply_filters('wp_multi_opening_hour_fields_backend', '',$day_key, $get_opening_hours, $time_list).'
	...
	public function restaurant_time_list($type_id = '') {
		...
		$time = date("h:i a", strtotime('+' . $lapse . ' minutes', $time));
                $time = apply_filters('foodbakery_time_format', $time);
		$hours[$time] = $time;
		$time = strtotime($time);
	...
	public function foodbakery_restaurant_save_opening_hours($restaurant_id = '') {
		...
		foreach ($days as $key => $day) {
			...
			update_post_meta($restaurant_id, 'foodbakery_opening_hours_' . $day . '_closing_time', $closing_time);
                        do_action('foodbakery_restaurant_save_opening_hours', $opening_hours_list, $restaurant_id, $day);
		    }
		}
	    }
	}
	...
	public function foodbakery_restaurant_categories($restaurant_id = '') {
	    $foodbakery_restaurant_cats = isset( $_POST['foodbakery_restaurant_category'] ) ? $_POST['foodbakery_restaurant_category'] : array();
            if( isset( $_POST['foodbakery_restaurant_category'] ) ){
                ...
                if ($foodbakery_restaurant_cats) {
                    foreach ($foodbakery_restaurant_cats as $foodbakery_restaurant_cat) {
                        ...
                        $restaurant_categories[]   = $term->slug;
                    }
                }
                update_post_meta( $restaurant_id, 'foodbakery_restaurant_category', $restaurant_categories );
