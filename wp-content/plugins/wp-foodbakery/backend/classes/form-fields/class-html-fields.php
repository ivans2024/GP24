	...
	class foodbakery_html_fields extends foodbakery_form_fields {
		...
		public function foodbakery_set_sub_heading($params = '') {
			...
			if ( isset($extra) ) {
				$extra_attr_html = $extra;
                        ...
                        $title2class    = isset( $title2class )? $title2class : '';
                        $title2 = isset( $title2)? '<div class="'.$title2class.'" style="float: right;background: #efefef;padding: 10px;color: #000;font-size: 13px;">'.$title2.'</div>' : '';
			$foodbakery_output .= '<div  id="' . $id . '" ' . $style . ' ' . $extra_attr_html . '>';
			$foodbakery_output .= '<div class="theme-header"><h1>' . esc_html($name) . $title2 . '</h1>
			</div>';
		...
		public function foodbakery_upload_file_field($params = '') {
			...
			if ( $html_render === true ) {
				...
				$foodbakery_output .= '</div><div class="col-lg-8 col-md-8 col-sm-12 col-xs-12 icon-image-browse-field">';
			}
			...
			$foodbakery_output .= '<div class="gal-edit-opts"><a href="javascript:;" class="delete delete_browse_image"></a> </div>';
			$foodbakery_output .= '</div>';
