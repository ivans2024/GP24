...
if (!class_exists('class_multiple_opening_hours_backend')) {

    class class_multiple_opening_hours_backend {

        /**
         * wp_moh_back_fields constructor.
         */
        function __construct() {
            add_filter('wp_multi_opening_hour_fields_backend', array($this, 'wp_multi_opening_hour_fields_backend_callback'), 10, 4);
            add_action('foodbakery_restaurant_save_opening_hours', array($this, 'foodbakery_restaurant_save_opening_hours_callback'), 10, 3);
         ...
         */
        public function foodbakery_restaurant_save_opening_hours_callback($opening_hours_list, $restaurant_id, $day){
        ...
        public function wp_multi_opening_hour_fields_backend_callback($empty_return,$day_key, $get_opening_hours, $time_list) {
            ...
            if (isset($get_opening_hours) && is_array($get_opening_hours) && sizeof($get_opening_hours) > 0) {
                ...
                if (is_array($time_list) && sizeof($time_list) > 0) {
                    ...
                    foreach ($time_list as $time_key => $time_val) {
                        $time_from_html .= '<option value="' . $time_key . '"' . ($opening_time == $time_key ? ' selected="selected"' : '') . '>' . date('h:i a', strtotime($time_val)) . '</option>' . "\n";
                        $time_to_html .= '<option value="' . $time_key . '"' . ($closing_time == $time_key ? ' selected="selected"' : '') . '>' . date('h:i a', strtotime($time_val)) . '</option>' . "\n";
                    }
                }
            ...
            $response = '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; and &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<select class="chosen-select opening_hours_fields" name="foodbakery_opening_hour[' . $day_key . '][opening_time_2]">
                    ' . $time_from_html . '
            </select>
    ...
    new class_multiple_opening_hours_backend();
}
?>