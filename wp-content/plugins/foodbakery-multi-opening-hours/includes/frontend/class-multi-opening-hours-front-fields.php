...
if (!class_exists('class_multi_opening_hours_front_fields')) {

    class class_multi_opening_hours_front_fields {
        ...
        public function wp_multi_opening_hour_input_fields_frontend_callback($empty_return,$day_key, $get_opening_hours, $time_list) {
            $day_status = isset($get_opening_hours[$day_key]['day_status']) ? $get_opening_hours[$day_key]['day_status'] : '';
                ...
                if (is_array($time_list) && sizeof($time_list) > 0) {
                    ...
                    foreach ($time_list as $time_key => $time_val) {
                        $time_from_html .= '<option value="' . $time_key . '"' . ($opening_time == $time_key ? ' selected="selected"' : '') . '>' . date_i18n('h:i a', strtotime($time_val)) . '</option>' . "\n";
                        $time_to_html .= '<option value="' . $time_key . '"' . ($closing_time == $time_key ? ' selected="selected"' : '') . '>' . date_i18n('h:i a', strtotime($time_val)) . '</option>' . "\n";
                    }
                }
    ...
    new class_multi_opening_hours_front_fields();
}
?>